...
const contactConfig = {"name": "Commercial Roofers of Odessa", "domain": "commercialroofersodessa.com", "address": "200 N Grant Ave, Suite 700, Odessa, TX 79761", "phone": "", "phoneTel": "", "email": "install@commercialroofersodessa.com", "city": "Odessa", "state": "Texas"};
...
module.exports = async function handler(req, res) {
  ...
  try { await sendLeadEmails(lead, req); } catch (error) { console.error("Contact email send failed", error); const message = error && error.message === "SENDGRID_API_KEY is missing." ? "Email service is not configured." : "We could not submit your request right now. Please email install@commercialroofersodessa.com."; return sendJson(res, 500, { ok: false, success: false, message, error: "email-send-failed" }, headers); }
